...
describe('Usando o método GET em /chocolates/search', function () {
  it('Retorna os chocolates baseados em nome', async function () {
    const response = await chai
      .request(app)
  ...
  it('Retorna um array vazio caso não seja encontrado os chocolates', async function () {
    const response = await chai
      .request(app)
      .get('/chocolates/search?name=AEHFIUAEFHAIUEH');
      expect(response.status).to.be.equal(404);
      expect(response.body).to.be.deep.equal([]);
